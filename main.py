...
def adjust_classes_engine(
    ...
    """
    단순 스왑 엔진(요청 반영):
    - 인원/성비/성적 균형을 '새로 맞추는' 로직 제거
    - 필요한 이동이 생기면(주로 떨어뜨리기 위반),
      "성별(=블록 남/여 구성) 동일 + 점수(블록 평균) 가장 유사"한 블록과 교환
    - 1대1 스왑을 반복하므로 여러 명 사이클 교환도 자연스럽게 발생 가능
    - 블록 남/여 구성이 동일한 것끼리만 스왑 -> 반별 성비/인원은 기본 반편성 그대로 유지
    """
    ...
    # (참고) 기존 하드룰 체크는 "진단용"으로만 남김
    cnt0 = class_counts_from_assignment(assignment, blocks, df_index, classes)
    diag.append(f"- initial hard-rules ok? {check_hard_rules(cnt0, size_min, size_max, gender_diff_max)}")
    diag.append(f"- initial violations: {len(violates_not_same(assignment, not_same_edges))}개")

    # 조건대상 블록(가능하면 이쪽 위주로 움직이기)
    ...
    diag.append(f"- movable blocks: {len(movable_blocks)}개 (조건대상 포함 블록)")
    ...
    def pick_move_block(bad_edges: List[Tuple[str, str]]) -> str:
        """위반 엣지에서 움직일 블록 하나 선택(조건대상 우선)."""
        a, b = bad_edges[0]
        if is_movable(a) and not is_movable(b):
            return a
        if is_movable(b) and not is_movable(a):
            return b
        # 둘 다 movable이거나 둘 다 아니면, 점수정보가 있는 쪽 우선
        _, _, _, avga = stats_cache[a]
        _, _, _, avgb = stats_cache[b]
        if avga is not None and avgb is None:
            return a
        if avgb is not None and avga is None:
            return b
        return a  # 기본

    def score_dist(bid1: str, bid2: str) -> float:
        return _score_distance(stats_cache, bid1, bid2)

    def same_gender_comp(bid1: str, bid2: str) -> bool:
        return _same_gender_comp(stats_cache, bid1, bid2)
    ...
    for it in range(max_iters):
        ...
        if cur_v == 0:
            diag.append(f"- SUCCESS: violations=0 (iters={it})")
            return assignment, "\n".join(diag)

        move_bid = pick_move_block(bad)
        move_cls = assignment[move_bid]

        # 후보: 다른 반 + 블록 남/여 구성 동일
        candidates: List[Tuple[Tuple[int, int, float], str]] = []
        # 정렬키: (after_violations, nonmovable_penalty, score_distance)
        for cand in blocks.keys():
            if cand == move_bid:
                continue
            cand_cls = assignment[cand]
            if cand_cls == move_cls:
                continue
            ...
            # "성비/인원 유지"를 위해: 남/여 구성 동일한 블록끼리만 스왑
            if not same_gender_comp(move_bid, cand):
                continue

            # 스왑 시뮬레이션
            assignment[move_bid], assignment[cand] = cand_cls, move_cls
            after_v = len(violates_not_same(assignment, not_same_edges))
            assignment[move_bid], assignment[cand] = move_cls, cand_cls

            nm_pen = (0 if is_movable(move_bid) else 1) + (0 if is_movable(cand) else 1)
            dist = score_dist(move_bid, cand)

            candidates.append(((after_v, nm_pen, dist), cand))

        if not candidates:
            diag.append(
                f"[ITER {it}] FAIL: move_bid={move_bid} 에 대해 '동일 남/여 구성' 스왑 후보가 없습니다. "
                f"(블록 묶기/떨어뜨리기 조합 때문에 구조적으로 막혔을 수 있음)"
            )
            raise ValueError("\n".join(diag))

        candidates.sort(key=lambda x: x[0])

        # 기본은 "위반을 늘리지 않는" 후보 우선, 없으면 최소 악화 허용(allow_non_improving_steps)
        ...
        if best_cand is None:
            if not allow_non_improving_steps:
                diag.append(f"[ITER {it}] FAIL: 모든 스왑이 위반을 증가시킴 (cur={cur_v})")
                raise ValueError("\n".join(diag))
            best_key, best_cand = candidates[0]
        ...
        after_v, nm_pen, dist = best_key
        if it % 20 == 0 or after_v < cur_v:
            diag.append(
                f"[ITER {it}] swap {move_bid}({move_cls}↔{cand_cls}) with {cand} "
                f"| violations {cur_v}->{after_v} | nm_pen={nm_pen} | score_dist={dist:.3f}"
            )
